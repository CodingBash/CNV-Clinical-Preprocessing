...
new_names <- c("chr", "start", "end", "cnlr")
names(facets_data) <- new_names
names(facets_bins_data) <- new_names

facets_bins_data <- na.omit(transform(facets_bins_data, start = c(0, start[-nrow(facets_bins_data)])))

print(facets_data)
print(facets_bins_data)

both_data <- rbind(facets_data, facets_bins_data)
print(both_data)
# print(data_bed)
...
gtrellis_layout(track_height = c(1,4), track_ylim = range(data.frame(facets_bins_data$cnlr)), nrow = 3, n_track = 2, byrow = FALSE, species="hg19")
...
add_segments_track(facets_bins_data, facets_bins_data$cnlr, gp = gpar(col = ifelse(facets_bins_data$cnlr > 0.2, "blue", ifelse(facets_bins_data$cnlr > -0.23, "black", "red")), lwd = 4))
